...
export function localAssistDraft(kind: AssistKind, prompt: string): string {
  ...
  switch (kind) {
    ...
    case "build.handoff":
      return [
        "Grok Build brief ready.",
        `Request: ${clipped}`,
        "Implement in the MyBizAI monorepo with a small diff. Do not re-ask the operator.",
        "Touch only the ADAPT loop / voice / assist surfaces needed. Summarize files changed.",
      ].join(" ");
    default:
      return clipped;
  }
...
function systemPrompt(kind: AssistKind): string {
  if (kind === "build.handoff") {
    return [
      "You package coding work for Grok Build (CLI coding agent).",
      "Return a tight implementation brief only — no full source code dumps.",
      "Include: goal, files/areas to touch, acceptance checks.",
      "Plain text, under 180 words.",
    ].join(" ");
  }
  return [
    ...
    "Never write large code blocks; if coding is required, say it should go to Grok Build.",
  ].join(" ");
}
